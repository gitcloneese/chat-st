...
func TestDeleteMany(t *testing.T) {
	...
	exTime := "02/23/2021 09:42:26"
	result, err := db.DeleteMany(context.Background(), collection, bson.M{"Status": 1, "Time": bson.M{"$lt": exTime}})
	if err != nil {
		t.Errorf("%v", err)
	}

	t.Logf("DeleteMany result %+v", result)
}
