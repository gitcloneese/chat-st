...
func Open(c *Config) (*DB, error) {
	...
	opts.SetMaxConnIdleTime(time.Duration(c.MaxConnIdleTime))
...
func (db *DB) CLose() error {
	if err := db.Disconnect(context.Background()); err != nil {
		log.Error("Mongo Disconnect err %v", err)
		return err
	}
	return nil
}
...
func (db *DB) DeleteMany(c context.Context, collection *mongo.Collection, filter interface{},
	opts ...*options.DeleteOptions) (result *mongo.DeleteResult, err error) {
	now := time.Now()
	defer slowLog(fmt.Sprintf("DeleteMany filter(%+v)", filter), now)
	if t, ok := trace.FromContext(c); ok {
		t = t.Fork(_family, "DeleteMany")
		t.SetTag(trace.String(trace.TagAddress, db.opts.Hosts[0]))
		defer t.Finish(&err)
	}
	if err = db.breaker.Allow(); err != nil {
		_metricReqErr.Inc(db.opts.Hosts[0], db.opts.Hosts[0], "DeleteMany", "breaker")
		return
	}
	_, c, cancel := db.conf.ExecTimeout.Shrink(c)
	result, err = collection.DeleteMany(c, filter, opts...)
	cancel()
	db.onBreaker(&err)
	_metricReqDur.Observe(int64(time.Since(now)/time.Millisecond), db.opts.Hosts[0], db.opts.Hosts[0], "DeleteMany")
	if err != nil {
		err = errors.Wrapf(err, "DeleteMany filter(%+v)", filter)
	}
	return
}
...
func (db *DB) InsertMany(ctx context.Context, collection *mongo.Collection, documents []interface{}, opts ...*options.InsertManyOptions) (result *mongo.InsertManyResult, err error) {
	...
	if err != nil {
		err = errors.Wrapf(err, "InsertMany filter(%+v)", interface{}(documents))
	}
	return
}
